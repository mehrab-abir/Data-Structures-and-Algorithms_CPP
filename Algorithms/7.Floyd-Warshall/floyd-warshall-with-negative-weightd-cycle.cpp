...
void floyd_warshall(vector<vector<int>>&adj_mat, int nodes) {
	...
	//Checking if there is any negative weighted cycle
	//if diagonal values are < 0, that means there are negative weighted cycle
	bool cycle = false;
	for (int i = 0; i < nodes; i++) {
		if (adj_mat[i][i] < 0) {
			cycle = true;
...
//graph with negative weighted cycle
/*
nodes 4, edges 5
0 1 3
2 0 -6
