...
void dfs(int src, vector<vector<int>>& adj_list, vector<bool>& visited, vector<int>& parent, bool& detectCycle) {
	...
	for (int adjacent : adj_list[src]) {
		...
		/*
		the adjacent node is already visited and it's not parent of the current(src) node, that means,
		going back to such a node which was previously visited but it's not the parent of the current
		(src) node either.
