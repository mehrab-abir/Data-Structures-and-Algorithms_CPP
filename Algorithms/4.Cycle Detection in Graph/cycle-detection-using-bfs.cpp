...
void bfs(int src, vector<vector<int>>&adj_list, vector<bool>&visited, vector<int>&parent, bool &detectCycle) {
	...
	while (!q.empty()) {
		...
		for (int adjacent : adj_list[current]) {
			...
			/*
			the adjacent node is visited and it's not parent of the current node, that means, going back
			to such a node which was previously visited but it's not the parent of current node either.
