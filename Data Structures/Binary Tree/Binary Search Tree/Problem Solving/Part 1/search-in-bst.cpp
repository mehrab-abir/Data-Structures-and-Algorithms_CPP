...
Node* searchInBST(Node* root, int searchNumber) {
	if (root == NULL) {
		return NULL;
	}
	...
	if (searchNumber < root->value) {
		root = searchInBST(root->left, searchNumber);
		return	root;
		
	}
	else {
		root = searchInBST(root->right, searchNumber);
		return root;
	}
}
...
int main() {
	...
	Node* foundRoot = searchInBST(root, searchNumber);

	if (foundRoot) {
		level_order(foundRoot);
	}
	else {
		cout << "Not found" << endl;
	}
